import React, { useState, useEffect } from "react";
import { StyleSheet,ActivityIndicator, View, Alert, Text } from "react-native";
import { ListItem, Avatar, Divider } from "react-native-elements";
import { ScrollView } from "react-native-gesture-handler";
import firebase from "../database/firebase";
import Moment from 'moment';
import 'moment/locale/es';
import Icon from 'react-native-vector-icons/FontAwesome';
import Button from 'apsl-react-native-button'
import auth from '@react-native-firebase/auth';

const CobrosDelDia = (props) => {
  const [pagos, setPagos] = useState([]);
  const [cuotasPagasBruno, setCuotasPagasBruno] = useState([]);
  const [cobradoBruno, setcobradoBruno] = useState(0);

  const [cuotasPagasOtro, setCuotasPagasOtro] = useState([]);
  const [cobradoOtro, setcobradoOtro] = useState(0);

  const [loading, setLoading] = useState(true);
  Moment.locale('es');
  const [date, setDate] = useState(Moment(new Date()).format('dddd DD/MM/yyyy'))

  useEffect(() => {
    firebase.db.collection("pagos").where("fechapagar", "==", date.toString()).orderBy("paga","asc").onSnapshot((querySnapshot) => {
      const pagos = [];
      querySnapshot.docs.forEach((doc) => {
        const { nombre, valor, paga, fechapagar, cuota, cobrador, fechacobro } = doc.data();
        pagos.push({
          id: doc.id,
          nombre,
          valor,
          paga,
          fechapagar,
          cuota,
          cobrador,
          fechacobro
        });
      });
      

      setCuotasPagasBruno(pagos.filter(pag  => pag.paga == "SI").filter(pag => pag.cobrador == "Bruno Costa"))
      //let sumBruno = cuotasPagasBruno.reduce(function(prev, current) {
      //  return prev + +current.valor
      //}, 0);
      //console.log(sumBruno)
      //setcobradoBruno(sumBruno)


      setCuotasPagasOtro(pagos.filter(pag  => pag.paga == "SI").filter(pag => pag.cobrador !== "Bruno Costa"))
      //let sumOtro = cuotasPagasOtro.reduce(function(prev, current) {
      //  return prev + +current.valor
      //}, 0);
      //console.log(sumOtro)
     // setcobradoOtro(sumOtro)
      
      setPagos(pagos);

      setLoading(false);


    });
  }, []);

  const pagarCuota =  async (cuota) => {

    const cuotaRef= firebase.db.collection("pagos").doc(cuota);
    const doc = await cuotaRef.get();
    const cuo = doc.data();
    const prest = cuo.idPrestamo;


    if(cuo.paga === "SI"){
      setLoading(false);
      Alert.alert("Error.", "La Cuota ya se ecuentra paga.")
      return;
    }
    Alert.alert(
        "Pagar Cuota de $"+ cuo.valor + " de "+ cuo.nombre,
        "Estás Seguro?",
        [
          { text: "Si", onPress: () => confirmaCuota(cuo.valor, cuotaRef, prest)}
          ,
          { text: "No", onPress: () => console.log("canceled") },
        ],
        {
          cancelable: true,
        }
      );

}

const confirmaCuota = async (cuotaVal, cuotaRef, prestamo) => {      

    setLoading(true);

    //console.log(cuota)
    await cuotaRef.update({
      paga: "SI",
      cobrador: auth().currentUser.displayName,
      fechacobro: date.toString()
    }).then(pagarPrestamo(cuotaVal, prestamo)) 



    setLoading(false);
    Alert.alert("Cuota Actualizada.", "La Cuota fue actualizada.")
    return false;
  };


  const pagarPrestamo = async (valor, prest) => {

    const dbRef = firebase.db.collection("prestamos").doc(prest);
    const doc = await dbRef.get();
    const pres = doc.data();


    await dbRef.update({

      deuda: (parseInt(pres.deuda) - parseInt(valor))

    })
    
    //const prest = doc.data();
    //setUser({ ...user, id: doc.id });
    
  };

  
  if (loading) {
    return (
      <View style={styles.loader}>
        <ActivityIndicator size="large" color="#9E9E9E" />
      </View>
    );
  }

  return (
    <View style={{flex:1, backgroundColor: '#f3f3f3'}}>     
    <ScrollView>
    <Divider 
      color='red'
      width={2} 
      orientation="horizontal"
      subHeader="Cuotas Impagas"
      subHeaderStyle={{ letterSpacing:0.25,alignSelf:'center', color: 'red', fontSize: 19, fontWeight:"bold" }}
      />
      <Divider
        color='red'
        width={2} 
        orientation="horizontal"
      />   
    {pagos.filter(pag  => pag.paga == "NO").map((pago) => {



        return (
          <ListItem
            key={pago.id}
            bottomDivider
            onPress= {()=> pagarCuota(pago.id)}
          >
          <ListItem.Chevron />  
            <Avatar 
              size={30}
              source={{
                uri:"data:image/png;base64," }}
             
            />
            <ListItem.Content>
              <ListItem.Title>Cliente: {pago.nombre} - Cuota: {pago.cuota}</ListItem.Title>
              <ListItem.Subtitle>Valor de la Cuota: ${pago.valor}</ListItem.Subtitle>
              <ListItem.Subtitle>Cuota Paga: {pago.paga}</ListItem.Subtitle>
              <ListItem.Subtitle>Fecha a Pagar: {pago.fechapagar}</ListItem.Subtitle>
              <ListItem.Subtitle>Cobrador: {pago.cobrador}</ListItem.Subtitle>
              <ListItem.Subtitle>Fecha de Cobro: {pago.fechacobro}</ListItem.Subtitle>
            </ListItem.Content>
          </ListItem>
        );
      })}
      <Divider
      color='white'
      width={25} 
      orientation="horizontal"
      /> 
      <Divider 
      color='green'
      width={2} 
      orientation="horizontal"
      subHeader="Cuotas Pagas"
      subHeaderStyle={{ alignSelf:'center', color: 'green', fontSize: 19, fontWeight:"bold" }}
      />
      <Divider
        color='green'
        width={2} 
        orientation="horizontal"
      />   
      {pagos.filter(pag  => pag.paga == "SI").map((pago) => {
        return (
          <ListItem
            key={pago.id}
            bottomDivider
            onPress= {()=> pagarCuota(pago.id)}
          >
          <ListItem.Chevron />  
          <Avatar 
            size={30}
            source={{
              uri: "data:image/png;base64,"
            }}
            
          />
            <ListItem.Content>
              <ListItem.Title>Cliente: {pago.nombre} - Cuota: {pago.cuota}</ListItem.Title>
              <ListItem.Subtitle>Valor de la Cuota: ${pago.valor}</ListItem.Subtitle>
              <ListItem.Subtitle>Cuota Paga: {pago.paga}</ListItem.Subtitle>
              <ListItem.Subtitle>Fecha a Pagar: {pago.fechapagar}</ListItem.Subtitle>
              <ListItem.Subtitle>Cobrador: {pago.cobrador}</ListItem.Subtitle>
              <ListItem.Subtitle>Fecha de Cobro: {pago.fechacobro}</ListItem.Subtitle>
            </ListItem.Content>
          </ListItem>
        );
      })}   

    </ScrollView>

    <Text style={styles.inputErr}> Total cobrado por Bruno:  <Icon style={styles.inputErr} size={25} name='usd' color='white'/>
          {cuotasPagasBruno.reduce(function(prev, current) {return prev + +current.valor}, 0)}
    </Text>
    <Text style={styles.inputErr}> Total cobrado por Otros:   <Icon style={styles.inputErr} size={25} name='usd' color='white'/> 
          {cuotasPagasOtro.reduce(function(prev, current) {return prev + +current.valor}, 0)}
    </Text>
    <Divider
        color='black'
        width={2} 
        orientation="horizontal"
      />  
    <Text style={styles.inputErr}> Total cobrado en el Día:    <Icon style={styles.inputErr} size={25} name='usd' color='white'/>
    {cuotasPagasBruno.reduce(function(prev, current) {return prev + +current.valor}, 0)+cuotasPagasOtro.reduce(function(prev, current) {return prev + +current.valor}, 0)}
    </Text>
    
    </View>
   
  );
};

const styles = StyleSheet.create({
  inputErr: {
    fontSize:18 ,
    color: 'black',
    letterSpacing: 0.25,

  },
  buttonGREY: {
    marginTop:30,
    flex: 1,
    marginBottom: 10,
    backgroundColor: '#E4FEFF',
    borderColor: 'white',
    borderRadius: 0,
  },
 
  searchIcon: {
    paddingLeft: 20,
    minWidth:60
    
  },
  searchIcon1: {
    padding : 10,
  },
  searchSection: {
    flex: 1,
    flexDirection: 'row',
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: '#fff',
    },
    fab: {
      position: 'absolute',
      margin: 16,
      right: 0,
      bottom: 0,
      backgroundColor: '#19AC52'
    },
    loader: {
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
      position: "absolute",
      alignItems: "center",
      justifyContent: "center",
    },
    avatar: {
      marginLeft:0,
      paddingLeft:0,
      backgroundColor: 'red'

    },
    

  });

export default CobrosDelDia;